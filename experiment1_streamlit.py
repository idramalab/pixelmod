...
# Milvus parameters
HOST = 'standalone'#'127.0.0.1'#change it to standalone for in-container usage 
PORT = '19530'
TOPK = 10
...
def display_search_results(results,imgname):
    ...
    if results is not None:
        ...
        st.image(filtered_images,width=200)
...
#img=Image.open("ManipulationSource/img22.jpg")
...
def trigger(clicked_index, image_title):
    st.write(f"Triggered! Image #{image_title} (index {clicked_index}) was clicked.")
...
def search_preloaded(image_content,imagename):
    ...
    if bytes_data is not None and len(bytes_data) > 0:
        ...
        display_search_results(image_search_results,imagename)
    else:
...
def slider_radius_changed():
    logging.info(f"Slider value changed {st.session_state.radius}")
    image_search_results = search_in_milvus(collection,st.session_state.bytes_data)
    display_search_results(image_search_results,st.session_state.imagename)
